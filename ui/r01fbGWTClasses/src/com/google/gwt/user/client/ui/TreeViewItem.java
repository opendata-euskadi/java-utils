...
public class TreeViewItem
  ...
  implements IsCollapsible,								// ...that can be collapsed / expanded
  			 HasTreeViewItems<TreeViewItem>,			// ...that has other tree view items as child items
  			 ...
  			 ForLazyLoadedObserver<ViewObject>,			// Observes view object's lazy load events
  			 CanPaint,									// Can paint a view object
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	@Delegate
	private final TreeViewIsHierarchicalDelegate<TreeViewItem> _childItemsContainerDelegate;
	...
	private final ObservableTreeViewItemDelegate _observableDelegate;
	@Override
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
	public TreeViewItem(final CanBePaintedAsWidget viewObjectToWidgetTransformer,
						final CanBePainted canBePainted) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void paint(final CanBePainted paintable) {
		...
		if (hierarchicalPaintable != null && hierarchicalPaintable.hasChildren()) {
			hasChild = true;
			Collection<CanBePainted> paintableChildren = CollectionUtils.cast(hierarchicalPaintable.getChildren());
	...
	@Override
	public void paintOverlay(final CanBePainted paintable) {
		// [1] - Set a reference to the view in the model
		//		 and store any view object's data at this view component
		paintable.paintedInto(this);
		this.setViewObjectData(paintable.getDataToStoreAtViewComponent());	// store some info about the painted object
		...
		if (paintable instanceof IsHierarchical) {
			...
			Collection<CanBePainted> childPaintables = CollectionUtils.cast(hierarchicalPaintable.getChildren());

			// Create NOT EXISTING nodes and recurse for ALREADY EXISTING nodes
			Collection<ViewObject> notExistingNodes = Lists.newArrayList();
			for (CanBePainted childPaintable : childPaintables) {
				TreeViewItem existingTreeViewItem = _findItemFor(this.getChildren(),
																 childPaintable);
				if (existingTreeViewItem != null) {
					existingTreeViewItem.paintOverlay(childPaintable);	// recurse
				} else {
					notExistingNodes.add((ViewObject)childPaintable);
				}
			}
	 ...
	 * @param children
	 */
	public void paintChildren(final Collection<ViewObject> children) {
		// the collection of child objects must be paintable
		Collection<CanBePainted> paintables = CollectionUtils.cast(children);
		_paintChildren(this,		// parent item
					   paintables,	// the child items
					   _viewObjectToWidgetTransformer);
		// TODO informar al widget de que se le han a�adido hijos...
	}
	private static TreeViewItem _findItemFor(final Collection<TreeViewItem> children,
											 final CanBePainted paintable) {
		TreeViewItem outItem = null;
		if (CollectionUtils.hasData(children)) {
			for (TreeViewItem childItem : children) {
				...
				if (childItem.getViewObjectStoredData() != null && paintable.getDataToStoreAtViewComponent() != null
				 && childItem.getViewObjectStoredData().equals(paintable.getDataToStoreAtViewComponent())) {
					outItem = childItem;
					break;
				}
			}
	...
	private static void _beginToObservePaintable(final TreeViewItem item,
												 final CanBePainted paintable) {
	...
	private static void _setStateFromPaintable(final TreeViewItem item,
											   final CanBePainted paintable) {
	...
	private static void _paintChildren(final TreeViewItem parentItem,
							    	   final Collection<CanBePainted> paintableChildren,
							    	   final CanBePaintedAsWidget viewObjectToWidgetTransformer) {
		if (CollectionUtils.hasData(paintableChildren)) {
			for (CanBePainted paintableChild : paintableChildren) {
				// check if the new child is already present as parent's child
				TreeViewItem previouslyExistingItem = _findItemFor(parentItem.getChildren(),
																   paintableChild);
				...
				TreeViewItem newChildItem = new TreeViewItem(viewObjectToWidgetTransformer);	// create the child item
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
	public final TreeView getTree() {
		return _tree;
	}
	void setTree(final TreeView newTree) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void expandUpToRoot() {
		TreeViewItem parentItem = this.getDirectAncestor();
