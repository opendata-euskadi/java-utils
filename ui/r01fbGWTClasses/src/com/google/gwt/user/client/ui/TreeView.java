...
public class TreeView 
  ...
  implements HasWidgets.ForIsWidget,
  			 HasValueChangeAtHandlers<TreeViewItem,Boolean>,HasSelectionHandlers<TreeViewItem>,
  			 HasOpenHandlers<TreeViewItem>,HasCloseHandlers<TreeViewItem>,
  			 HasTreeViewItemCollapseHandlers<TreeViewItem>,HasTreeViewItemExpandHandlers<TreeViewItem>,
  			 HasClickHandlers,
  			 HasAllKeyHandlers,
  			 HasAllMouseHandlers,
  			 HasAllFocusHandlers,
  			 ...
  			 HasTreeViewItems<TreeViewItem>,				// has tree view items
  			 TreeViewItemsContainCapable,					// ... so can contain child tree view items
  			 CanPaint {  									// Can paint view objects (it's a view component)
...
//  ==== Fields for WidgetReferences =========================================================
	 ...
	 */
	private final Map<Widget,TreeViewItem> _descendantWidgets = new HashMap<Widget,TreeViewItem>();
...
//  ==== Fields use when managing events for keyboard navigation =============================
	 ...
	 * Item with the focus
	 */
	private TreeViewItem _focusedItem;
	/**
	 ...
	 */
	private TreeViewItem _relatedEventsTargetItem;
	/**
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	@Override
	public <T> void setViewObjectData(final T data) {
		_data = data;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
	private final CanBePaintedAsWidget _treeViewObjectToWidgetTransformer;
	/**
	 * Transforms a {@link CanBePainted} {@link ViewObject} to a GWT {@link Widget}
	 * (it's used to paint every child item)
	 */
	private final CanBePaintedAsWidget _itemViewObjectToWidgetTransformer;
	
	@Delegate
	private final TreeViewIsHierarchicalDelegate<TreeView> _childItemsContainerDelegate;
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
	public TreeView(final CanBePaintedAsWidget treeViewObjectToWidgetTransformer,
					final CanBePaintedAsWidget itemViewObjectToWidgetTransformer) {
		...
		// Delegates
		_childItemsContainerDelegate = new TreeViewIsHierarchicalDelegate<TreeView>(_itemViewObjectToWidgetTransformer,
																					this);
...
//	}
	...
	@Override  
	public void paint(final CanBePainted paintable) {
		// [1] - Set a reference to the view in the model
		//		 and store any view object's data at this view component
		if (paintable != null) {
			paintable.paintedInto(this);
			this.setViewObjectData(paintable.getDataToStoreAtViewComponent());	// store some info about the painted object
		}
	...
	private void _paintPaintables(final Collection<CanBePainted> paintables,
								  final boolean overlay) {
		CollectionUtils.executeOn(paintables,
								  new CommandOn<CanBePainted>() {
										@Override
										public void executeOn(final CanBePainted paintable) {
											TreeViewItem existingTreeItem = overlay ? _alreadyExistsItem(paintable,TreeView.this.getChildren())
																					: null;
											if (existingTreeItem == null) {
												TreeViewItem rootItem = new TreeViewItem(_itemViewObjectToWidgetTransformer);	// Create the root item											
	...
	private static Collection<CanBePainted> _paintableChildNodesOf(final CanBePainted modelObj) {
		Collection<CanBePainted> outPaintables = null;
		if (modelObj instanceof HasChildren<?>) {
			HasChildren<?> modelObjWithChildren = (HasChildren<?>)modelObj;
	...
	private static TreeViewItem _alreadyExistsItem(final CanBePainted paintable,
												   final Collection<TreeViewItem> items) {
		TreeViewItem outExistingTreeViewItem = null;
		if (CollectionUtils.hasData(items)) {
			for (TreeViewItem item : items) {
				...
				if (item.getViewObjectStoredData() != null && paintable.getDataToStoreAtViewComponent() != null
				 && item.getViewObjectStoredData().equals(paintable.getDataToStoreAtViewComponent())) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	void adopt(final TreeViewItem treeItem) {
	 ...
	 * @param widget
	 */
	void orphan(final TreeViewItem treeItem) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
	Map<Widget,TreeViewItem> getChildWidgets() {
...
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * @return
	 */
	public Collection<TreeViewItem> findItemsWithData(final Object data) {
		if (data == null) return null;
		Collection<TreeViewItem> outItems = Lists.newArrayList();
		_accumItemsWithData(data,
							this.getChildren(),
							outItems);
		return CollectionUtils.hasData(outItems) ? outItems : null;
	}
	private void _accumItemsWithData(final Object data,final Collection<TreeViewItem> items,
									final Collection<TreeViewItem> accum) {
		if (CollectionUtils.hasData(items)) {
			for (TreeViewItem item : this.getChildren()) {
				if (item.getViewObjectStoredData() != null && item.getViewObjectStoredData().equals(data)) {
					accum.add(item);
				}
				// recurse
				_accumItemsWithData(data,
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	private void _onFocus(final TreeViewItem item) {		
		_focusedItem = item;
		if (_focusedItem != null) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	protected boolean isKeyboardNavigationEnabled(final TreeViewItem currentItem) {
		return true;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	private void _expandChildNodeset(final Collection<TreeViewItem> childItems,
									 final boolean onlyLoaded) {
		if (CollectionUtils.hasData(childItems)) {
			for (TreeViewItem item : childItems) {
	...
	public void ensureSelectedItemVisible() {
		...
		TreeViewItem parent = _focusedItem.getDirectAncestor();
		while (parent != null) {
			parent.expand();
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	@Override
	public HandlerRegistration addValueChangeAtHandler(final ValueChangeAtHandler<TreeViewItem,Boolean> handler) {
		return this.addHandler(handler,
							   ValueChangeAtEvent.getTYPE());
	}
	@Override
	public HandlerRegistration addSelectionHandler(final SelectionHandler<TreeViewItem> handler) {
		return this.addHandler(handler,
							   SelectionEvent.getType());
	}
	@Override
	public final HandlerRegistration addItemCollapseHandler(final TreeViewItemCollapseHandler<TreeViewItem> handler) {
		return this.addHandler(handler,
							   TreeViewItemCollapseEvent.getTYPE());
	}
	@Override
	public HandlerRegistration addItemExpandHandler(final TreeViewItemExpandHandler<TreeViewItem> handler) {
		return this.addHandler(handler,
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void add(final Widget w) {
		TreeViewItem item = new TreeViewItem(_itemViewObjectToWidgetTransformer);
		item.setWidget(w);
		this.addItem(item);
	}
	@Override
	...
	public boolean remove(final Widget w) {
		TreeViewItem item = _descendantWidgets.get(w);
