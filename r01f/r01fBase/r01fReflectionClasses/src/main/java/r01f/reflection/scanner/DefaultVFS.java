...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    @Override
    public List<String> _list(final URL url,
        ...
        try {
            ...
            URL jarUrl = _findJarForResource(url);
            if (jarUrl != null) {
                is = jarUrl.openStream();
                log.debug("Listing {}", url);
                resources = _listResources(new JarInputStream(is), path);
            }
            else {
                ...
                try {
                    if (_isJar(url)) {
                        ...
                        for (String line; (line = reader.readLine()) != null;) {
                            ...
                            lines.add(line);
                            if (_getResources(path + "/" + line).isEmpty()) {
                                lines.clear();
                                break;
                            }
                        }
                    ...
                    if ("file".equals(url.getProtocol())) {
                        ...
                        throw e;	  // No idea where the exception came from so rethrow it
                    }
                }
                ...
                for (String child : children) {
                    ...
                    resources.addAll(_list(childUrl, resourcePath));
                }
            }
            return resources;
        } finally {
...
/////////////////////////////////////////////////////////////////////////////////////////
     ...
     */
    protected static List<String> _listResources(final JarInputStream jar, 
    									 		 final String path) throws IOException {
        // Include the leading and trailing slash when matching names
    	String thePath = path;
        if (!path.startsWith("/")) thePath = "/" + thePath;
        if (!path.endsWith("/")) thePath = thePath + "/";
     ...
     */
    protected static URL _findJarForResource(final URL url) throws MalformedURLException {
        ...
        StringBuilder jarUrl = new StringBuilder(theUrl.toExternalForm());
        ...
        try {
            ...
            if (_isJar(testUrl)) return testUrl;

            // WebLogic fix (exploded ear deployment): check if the URL's file exists in the filesystem.
            log.trace("Not a JAR: {}",jarUrl);
...
/////////////////////////////////////////////////////////////////////////////////////////
     ...
     */
    protected static boolean _isJar(final URL url) {
        return _isJar(url,new byte[JAR_MAGIC.length]);
    }
    /**
     * Returns true if the resource located at the given URL is a JAR file.
     * 
	...
	protected static boolean _isJar(final URL url,final byte[] buffer) {
        InputStream is = null;
        try {
            is = url.openStream();
