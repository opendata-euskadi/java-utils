...
import r01f.httpclient.jsse.security.x509.CertificateIssuerName;
import r01f.httpclient.jsse.security.x509.X500Name;
import r01f.httpclient.jsse.security.x509.X509CRLImpl;
import r01f.httpclient.jsse.security.x509.X509CertImpl;
import r01f.httpclient.jsse.security.x509.X509CertInfo;
...
public class PKCS7 {
     ...
     * @exception ParsingException on parsing errors.
     */
    public PKCS7(DerInputStream derin) throws Exception  {
        parse(derin);
    }
     ...
     * @exception ParsingException on parsing errors.
     */
    public PKCS7(byte[] bytes) throws Exception {
        try {
     ...
     */
    private void parse(DerInputStream derin)  throws IOException
    {
        try {
            ...
            try {
                ...
                IOException pe = new IOException(ioe1.getMessage());
                pe.initCause(ioe1);
                throw pe;
            }
        }
    ...
    throws IOException {
        ...
        for (int i=0; i < contents.length; i++) {
            ...
            try {
                ...
                IOException pe = new IOException(ce.getMessage());
                pe.initCause(ce);
                throw pe;
            } catch (IOException ioe) {
                IOException pe = new IOException(ioe.getMessage());
                pe.initCause(ioe);
                throw pe;
            } finally {
                if (bais != null)
    ...
    private void parseOldSignedData(DerValue val)
        ...
        try {
            ...
            throw new IOException("Error parsing digest AlgorithmId IDs");
        }
